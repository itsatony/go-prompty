...
### Added
- **`ProjectID`** field on `SkopeConfig`: Singular deployment-target project identifier (e.g., GCP project, Azure subscription)
- **`Regions`** field on `SkopeConfig`: Allowed deployment regions as `[]string` (e.g., `["us-east-1", "eu-west-1"]`)
- New accessor methods: `GetProjectID()`, `HasProjectID()`, `GetRegions()`, `HasRegions()`, `AddRegion()`
- Validation: empty strings rejected in `Regions` slice
